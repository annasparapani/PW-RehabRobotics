...
Data_train_allTogether = [A; B; C];

% Test dataset
D = T1(round(length(T1)*0.8):end,:); 
E = T2(round(length(T2)*0.8):end,:); 
F = T3(round(length(T3)*0.8):end,:); 
Data_test_allTogether = [D; E; F]; 

%save('Data_train.mat') % saves the workspace
%save('Data_test.mat')

writematrix(Data_test_allTogether, "data_test_allSubs.csv") % saves the matrix as a csv file
writematrix(Data_train_allTogether, "data_train_allSubs.csv")
